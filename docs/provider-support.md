...
|---|---|---|---|
| **OpenAI** | **Available:** exact `o200k_base` and `cl100k_base` raw-text counts, plus a dated registry of exact model aliases. | **Planned:** separately trained and evaluated per tokenizer family. | **Available for GPT-5 raw text:** pinned USD input, cached-input, and output rates; more models and caller overrides are planned. |
