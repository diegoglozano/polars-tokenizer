...
|---|---|---|---|
| **OpenAI** | **Available (tokenizer-level):** exact `o200k_base` and `cl100k_base` raw-text counts. Versioned model aliases are planned. | **Planned:** separately trained and evaluated per tokenizer family. | **Planned:** first registry target; model aliases will resolve tokenizer plus dated input, cached-input, and output prices. |
