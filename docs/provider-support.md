...
|---|---|---|---|
| **OpenAI** | **Available (partial):** `o200k_base` raw-text counts. `cl100k_base` and versioned model aliases are planned. | **Planned:** separately trained and evaluated per tokenizer family. | **Planned:** first registry target; model aliases will resolve tokenizer plus dated input, cached-input, and output prices. |
